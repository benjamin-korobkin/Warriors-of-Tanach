...
func _ready() -> void:
	...
	card_label_min_sizes["Description"] = Vector2(CFConst.CARD_SIZE.x - 4,STANDARD_FONT_SIZE + 4)
	card_labels["CardID"] = find_node("CardID")
	card_label_min_sizes["CardID"] = Vector2(CFConst.CARD_SIZE.x - 4,STANDARD_FONT_SIZE - 2)
